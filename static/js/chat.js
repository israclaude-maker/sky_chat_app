    ...
    if (e.track.kind === 'video') {
      ...
      if (hasExistingCamera) {
        ...
        e.track.onended = function () {
          ...
          var rv = $('remote-video');
          if (rv) { rv.classList.remove('screen-pip'); rv.style.cssText = ''; }
        };
      } else {
  ...
  if (data.sharing) {
    ...
    if (!lbl) {
      ...
      lbl.innerHTML =
        ...
        esc(CallState.remoteUserName) + ' is sharing screen';
...
function showOngoingCall() {
  ...
  var remoteScreenVideo = $('remote-screen-video');
  if (remoteScreenVideo) { remoteScreenVideo.style.display = 'none'; remoteScreenVideo.srcObject = null; }
  var localVid = $('local-video');
  ...
  if (CallState.callType === 'video') {
    ...
    if (remoteVideo) {
      remoteVideo.style.display = 'block';
      if (CallState.remoteStream) remoteVideo.srcObject = CallState.remoteStream;
    }
  } else {
...
function startScreenShare() {
  ...
  navigator.mediaDevices
    .getDisplayMedia({ video: true, audio: false, monitorTypeSurfaces: 'include' })
    ...
    .catch(function (err) { console.log('Screen share cancelled or denied:', err); });
}
...
function cleanupCall() {
  ...
  if (lbl) lbl.remove();
  var rv = $('remote-video');
  if (rv) { rv.classList.remove('screen-share'); rv.classList.remove('screen-pip'); rv.style.cssText = ''; rv.srcObject = null; }
  var rsv = $('remote-screen-video');
  if (rsv) { rsv.style.display = 'none'; rsv.srcObject = null; }
  var lv = $('local-video');
  if (lv) { lv.style.cssText = ''; lv.srcObject = null; }
  ...
  if (videoTracks.length > 0 && !GC.isCamOff) {
    ...
    videoTracks.forEach(function (t) {
      ...
      Object.keys(GC.peers).forEach(function (pid) {
        ...
        for (var i = 0; i < senders.length; i++) {
          if (senders[i].track === t) { peer.pc.removeTrack(senders[i]); break; }
        }
      });
    });
    ...
    Object.keys(GC.peers).forEach(function (pid) {
      ...
      peer.pc.createOffer()
        ...
        .catch(function (e) { console.error('GC cam off renegotiate:', e); });
    });
    syncGcButtonStates();
    buildLocalThumb();
    if (gcFocusedId === 'local') focusGcParticipant('local');
    ...
    navigator.mediaDevices.getUserMedia({
      ...
      Object.keys(GC.peers).forEach(function (pid) {
        ...
        peer.pc.createOffer()
          ...
          .catch(function (e) { console.error('GC cam on renegotiate:', e); });
      });
      syncGcButtonStates();
      buildLocalThumb();
      if (gcFocusedId === 'local') focusGcParticipant('local');
...
function handleGroupCallAnswer(data) {
  ...
  peer.pc.setRemoteDescription(new RTCSessionDescription(data.sdp)).then(function () {
    ...
    if (peer.pendingIce && peer.pendingIce.length) {
      console.log('[GC] Flushing', peer.pendingIce.length, 'ICE for peer', fromId);
...
function handleGroupCallIce(data) {
  ...
  if (peer.pc && peer.pc.remoteDescription && peer.pc.remoteDescription.type) {
    // Remote description set hai — seedha add karo
    peer.pc.addIceCandidate(new RTCIceCandidate(data.candidate)).catch(function (e) {
...
function createGroupPeerConnection(peerId) {
  ...
  var peer = {
    ...
    stream: null,
    pendingIce: [],
    _pendingScreenStream: null   // ← YEH NAYI LINE HAI
  };
  ...
  if (GC.pendingIceByUser && GC.pendingIceByUser[peerId]) {
    ...
    console.log('[GC] Transferred', peer.pendingIce.length, 'queued ICE for', peerId);
  }
  ...
  // Screen share tracks bhi add karo agar chal rahi hai
...
function setupGroupPeerHandlers(pc, peer, peerId) {
  ...
  pc.ontrack = function (event) {
    console.log('[GC] ontrack for peer', peerId, 'track:', event.track.kind,
      'streams:', event.streams.length, 'stream id:', event.streams[0] && event.streams[0].id);

    var incomingStream = event.streams[0];

    // Stream nahi aayi — manually banao
    if (!incomingStream) {
      if (!peer.stream) {
        peer.stream = new MediaStream();
        peer.stream._gcManual = true;
      }
      if (!peer.stream.getTrackById(event.track.id)) {
        peer.stream.addTrack(event.track);
      }
      event.track.onunmute = function () { renderGroupCallPeer(peerId, peer); };
      renderGroupCallPeer(peerId, peer);
      return;
    }

    // Pehli stream — camera/audio
    if (!peer.stream) {
      peer.stream = incomingStream;
      peer.stream.onaddtrack = function () { renderGroupCallPeer(peerId, peer); };
      peer.stream.onremovetrack = function () { renderGroupCallPeer(peerId, peer); };
    } else if (peer.stream.id === incomingStream.id) {
      // Same stream — koi masla nahi
      peer.stream = incomingStream;
    } else {
      // Alag stream ID — screen share ki POSSIBILITY hai
      // Lekin confirm gc_screen_toggle se hoga — abhi pending rakh
      if (!peer._pendingScreenStream) {
        peer._pendingScreenStream = incomingStream;
        console.log('[GC] Pending screen stream stored for peer', peerId);
      }
    }

    // Track state listeners — re-render trigger
    event.track.onunmute = function () {
      console.log('[GC] Track unmuted for peer', peerId, event.track.kind);
      renderGroupCallPeer(peerId, peer);
    };
    event.track.onmute = function () { renderGroupCallPeer(peerId, peer); };
    event.track.onended = function () { renderGroupCallPeer(peerId, peer); };
  ...
  pc.onconnectionstatechange = function () {
    ...
    if (pc.connectionState === 'connected') {
      // Connected hote hi render karo
      renderGroupCallPeer(peerId, peer);
      // Talking detection update karo
      ...
      // 5 seconds baad agar wapas nahi aaya to cleanup
      setTimeout(function () {
        if (peer.pc && peer.pc.connectionState === 'disconnected') {
          console.log('[GC] Peer', peerId, 'still disconnected — restarting ICE');
          peer.pc.restartIce();
        }
      }, 5000);
    }
...
function renderGroupCallPeer(peerId, peer) {
  console.log('[GC] renderGroupCallPeer:', peerId,
    'stream:', !!peer.stream,
    'videoTracks:', peer.stream ? peer.stream.getVideoTracks().length : 0,
    'connectionState:', peer.pc ? peer.pc.connectionState : 'N/A');
...
function handleGcScreenToggle(data) {
  ...
  if (sharing) {
    ...
    if (peer) {
      // Pending screen stream use karo jo ontrack ne store kiya tha
      if (peer._pendingScreenStream) {
        peer.screenStream = peer._pendingScreenStream;
        peer._pendingScreenStream = null;
        console.log('[GC] Activated pending screen stream for peer', fromId);
      }
